    ...
    async def handle_message(
        ...
        if message.type == "text" and message.text:
            ...
            if match:
                ...
                product = await self.product_service.product_repo.get_by_fallback_id(
                    fallback_id
                )
                if not product:
                    await whatsapp_client.send_text_message(
                        to=sender_number,
                        text="Sorry, we couldn't find a product matching this code.",
                    )
                    return
                ...
                stock_line = (
                    ...
                    else "📦 Not Available: Out of Stock"
                )
                message_text = (
                ...
                if product.inventory > 0:
                    ...
                    await whatsapp_client.send_interactive_buttons(
                        to=sender_number, text=message_text, buttons=buttons
                    )
                else:
                    await whatsapp_client.send_text_message(
                        to=sender_number, text=message_text
                    )
                return
        ...
        if (
            message.type == "text"
            and message.text
            and customer.conversation_state == "AWAITING_ADDRESS"
        ):
            ...
            await self.customer_service.update_delivery_address_text(
                customer.id, address_text
            )
            ...
            await whatsapp_client.send_interactive_buttons(
                to=sender_number, text=text, buttons=buttons
            )
            return
        ...
        if (
            ...
            if payload.startswith("add_to_cart_"):
                ...
                try:
                    ...
                    await whatsapp_client.send_interactive_buttons(
                        to=sender_number, text=str(e.message), buttons=buttons
                    )
                except Exception as e:
                    print(f"Error adding to cart: {e}")
                    await whatsapp_client.send_text_message(
                        to=sender_number,
                        text="Sorry, there was an issue adding this item to your cart.",
                    )
                return
            elif payload == "add_more_items":
                await whatsapp_client.send_text_message(
                    to=sender_number,
                    text="📸 Please scan or upload another product's QR code to add it to your cart.",
                )
                return
                ...
                if "empty" in summary.lower():
                    await whatsapp_client.send_text_message(
                        to=sender_number, text=summary
                    )
                else:
                    await whatsapp_client.send_interactive_buttons(
                        to=sender_number, text=summary, buttons=buttons
                    )
                return
            elif payload == "checkout":
                if not customer.delivery_address:
                    await self.customer_service.update_conversation_state(
                        customer.id, "AWAITING_ADDRESS"
                    )
                    await whatsapp_client.send_text_message(
                        to=sender_number,
                        text="📍 Please type your delivery address to proceed with the checkout.",
                    )
                    return
                ...
                address = (
                    customer.delivery_address.get("text", "Saved Address")
                ...
                await CartService.clear_cart(customer.id)
                await whatsapp_client.send_text_message(
                    to=sender_number,
                    text="🗑️ Your cart has been cleared.📸 Please scan or upload another product's QR code to add it to your cart.",
                )
                return
                ...
                await whatsapp_client.send_text_message(
                    to=sender_number,
                    text="📍 Please type your new delivery address to proceed with the checkout.",
                )
                return
        ...
        if message.type == "image" and message.image:
            ...
            if image_url:
                try:
                    ...
                    await whatsapp_client.send_text_message(
                        to=sender_number,
                        text="Sorry, we couldn't download the image right now.",
                    )
                    return
                ...
                if img is not None:
                    ...
                    if decoded_qr:
                        ...
                        if match:
                            ...
                            if not product:
                                print(
                                    f"Product not found for fallback ID: {fallback_id}"
                                )
                                await whatsapp_client.send_text_message(
                                    to=sender_number,
        ...
        await whatsapp_client.send_text_message(
            to=sender_number, 
            text="📸 Please scan or upload product's QR code."
        )
